...
module.exports = {
  ...
  addLog(gameId, log, date, cb) {
    let key = keyLogs(gameId)
    let args = [key, date, log]
  ...
  addMessage(gameId, message, date, cb) {
    let key = keyChat(gameId)
    let args = [key, date, message]
    client.zadd(args, (err) => {
      if (err) return cb(err)
      cb()
    })
  ...
  getMessages(gameId, cb) {
    let key = keyChat(gameId)
    let args = [key, 0, -1, 'WITHSCORES']
    client.zrange(args, (err, messages) => {
      if (err) return cb(err)
      cb(null, messages)
    });
  },
