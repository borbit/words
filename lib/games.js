  ...
  addGame(user1FBId, user2FBId, cb) {
    async.auto({
      ...
      game: ['user1', 'user2', 'user1Games', (cb, data) => {
        ...
        let unfinished = _.find(data.user1Games, (game) => {
          return game.opponent.fb_id == user2FBId && !game.finished_at
        })
        if (unfinished) {
          return cb(new Error(`You have unfinished games with ${unfinished.opponent.fb_name}`))
        }
  ...
  playGame(gameId, userFBId, letters, cb) {
    async.auto({
      ...
      play: ['words', (cb, data) => {
        ...
        let prevLetters = _.filter(_.flatten(data.words.cells), (c) => {
